...
.stApp {
    background:
        radial-gradient(circle at 20% 0%, rgba(233,247,236,.75), transparent 26%),
        radial-gradient(circle at 95% 5%, rgba(255,238,222,.9), transparent 28%),
        #fbf7ef;
    color:#173b36;
}
...
.sidebar-icon {
    width:32px;height:32px;border-radius:10px;background:#e6f5e8;color:#0d7a54;
...
.table-title { color:#0b3f3b;font-size:18px;font-weight:850; }
.toolbar-actions { display:flex;gap:10px;color:#425a52;font-size:13px;font-weight:700; }
...
.card.green { background:linear-gradient(135deg,#f4fbf4,#fffdf9); }
.card.blue { background:linear-gradient(135deg,#f4f9ff,#fffdf9); }
.card.gold { background:linear-gradient(135deg,#fff8e8,#fffdf9); }
.card.red { background:linear-gradient(135deg,#fff2f2,#fffdf9); }
...
def load_catalog():
    try:
        ...
        for g in games:
            ...
            info = {
                "thumb": g.get("urlThumb", "") or g.get("urlImage", ""),
                "image": g.get("urlImage", "") or g.get("urlThumb", ""),
            ...
            if bgg_id:
                by_id[bgg_id] = info
            if g.get("name"):
                by_name[g["name"].strip().lower()] = info
...
def load_data():
    ...
    if "Jugadores" not in df.columns:
        def players(row):
            c = row["_catalog"]
            if c.get("minPlayers") and c.get("maxPlayers"):
                return f"{c.get('minPlayers')}–{c.get('maxPlayers')}"
            return ""
    ...
    if "Duración" in df.columns:
        ...
        def time_text(row):
            c = row["_catalog"]
            mn, mx = c.get("minPlayTime"), c.get("maxPlayTime")
...
def chart_layout(fig, h=190):
    fig.update_layout(
        height=h,
        margin=dict(l=4, r=4, t=4, b=4),
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        font=dict(size=11, color="#65706d"),
        showlegend=False,
    )
...
metrics = [
    (c1, "green", "▣", total, "Juegos en ludoteca"),
    (c2, "blue", "▽", filtered, "Juegos filtrados"),
    (c3, "gold", "★", unplayed, "Sin estrenar"),
    (c4, "purple", "ϟ", low_friction, "Baja fricción"),
    (c5, "peach", "◷", old_6, "Hace +6 meses"),
    (c6, "rose", "♡", favorites, "Favoritos"),
]
for col, color, icon, value, label in metrics:
    with col:
        st.markdown(
            f'<div class="metric-card"><div class="metric-icon {color}">{icon}</div><div><div class="metric-value">{value}</div><div class="metric-label">{label}</div></div></div>',
            unsafe_allow_html=True,
        )
...
<div class="table-toolbar">
    ...
    <div class="toolbar-actions"><span>▦ Columnas</span><span>↧ Exportar</span></div>
</div>
""",
    unsafe_allow_html=True,
)
...
table["Fricción"] = table["Fricción para mesa"].apply(friction_chip)
...
st.dataframe(
    ...
    column_config={
        ...
        "Juego": st.column_config.TextColumn("Juego", width="large"),
...
with p1:
    st.markdown(f'<div class="card green"><div class="card-title">Sin estrenar ({len(f[f["Partidas_num"] == 0])})</div>{small_list(sin_estrenar)}</div>', unsafe_allow_html=True)
with p2:
    st.markdown(f'<div class="card blue"><div class="card-title">Menos jugados</div>{small_list(menos_jugados)}</div>', unsafe_allow_html=True)
with p3:
    st.markdown(f'<div class="card gold"><div class="card-title">Hace más de 6 meses ({len(f[f["Ultima_dt"].notna() & (f["Ultima_dt"] < today - pd.DateOffset(months=6))])})</div>{small_list(mas_6)}</div>', unsafe_allow_html=True)
with p4:
    st.markdown(f'<div class="card green"><div class="card-title">Jugados recientemente</div>{small_list(recientes)}</div>', unsafe_allow_html=True)
with p5:
    st.markdown('<div class="chart-card"><div class="chart-title">Distribución por fricción</div>', unsafe_allow_html=True)
    ...
    if len(fr):
        fig = px.pie(
            ...
            hole=0.55,
            color="Fricción",
            color_discrete_map={"Baja":"#69c184", "Media":"#f0a440", "Alta":"#ef6b5f", "Muy alta":"#d85c5c", "Sin dato":"#d8d2c8"},
        )
        fig.update_layout(
            height=185,
...
with g1:
    st.markdown('<div class="chart-card"><div class="chart-title">Por tipo</div>', unsafe_allow_html=True)
    if len(f):
...
with g2:
    st.markdown('<div class="chart-card"><div class="chart-title">Por jugadores</div>', unsafe_allow_html=True)
    if len(f):
...
with g3:
    st.markdown('<div class="chart-card"><div class="chart-title">Por tiempo de juego</div>', unsafe_allow_html=True)
    if len(f):
...
with g4:
    st.markdown('<div class="chart-card"><div class="chart-title">Por peso BGG</div>', unsafe_allow_html=True)
    if len(f):
        bins = {
